 ...
 * Runtime: 652 ms
 * Memory: 53.2 MB
 * Question Url: https://leetcode.com/problems/open-the-lock
 * Submission Url:
 * https://leetcode.com/problems/open-the-lock/submissions/502841834
 */
...
public:
  ...
  vector<string> findAllPaths(string root, set<string> &vis,
                              set<string> &deadSet) {
    vector<string> paths;
    for (int i = 0; i < 4; i++) {
      ...
      if (vis.find(new_path) == vis.end() and
          deadSet.find(new_path) == deadSet.end()) {
