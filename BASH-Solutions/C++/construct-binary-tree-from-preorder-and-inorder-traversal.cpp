 ...
 * Submission Url:
 * https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal/submissions/746720925
 */
...
public:
  ...
  TreeNode *fun(int index, int lm, int rm, vector<int> &preorder,
                unordered_map<int, int> &mp) {
    ...
    if (inorder_index < rm) {
      node->right = fun(index + (inorder_index - lm) + 1, inorder_index + 1, rm,
                        preorder, mp);
    }
