...
Submission Url: https://leetcode.com/problems/out-of-boundary-paths/submissions/748260578
*/
...
public:
    int findPaths(int m, int n, int maxMove, int startRow, int startColumn) {
        ...
        for(int i = 1; i <= maxMove; i++){
            ans += solve(m, n, startRow, startColumn, i, mp) % 1000000007;
        }
    ...
    int solve(int m, int n, int x, int y, int move, map<string, int> &mp) {
        ...
        if(canMove(m,n,x-1,y)){
            ans += solve(m,n,x-1,y,move-1, mp) % 1000000007;
        }
        if(canMove(m,n,x+1,y)){
            ans += solve(m,n,x+1,y,move-1, mp) % 1000000007;
        }
        if(canMove(m,n,x,y-1)){
            ans += solve(m,n,x,y-1,move-1, mp) % 1000000007;
        }
        if(canMove(m,n,x,y+1)){
            ans += solve(m,n,x,y+1,move-1, mp) % 1000000007;
        }
