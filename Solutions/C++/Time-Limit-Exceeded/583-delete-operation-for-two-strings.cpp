...
Submission Url: https://leetcode.com/problems/delete-operation-for-two-strings/submissions/489957561
*/
...
public:
    ...
    int fun(string s1, string s2, int i, int j, vector<vector<int>> &dp){
        ...
        if(i >= s1.size()){
            return fun(s1, s2, i, j+1, dp) + 1;
        }
        if(j >= s2.size()){
            return fun(s1, s2, i+1, j, dp) + 1;
        }
