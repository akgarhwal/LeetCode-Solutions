...
Submission Url: https://leetcode.com/problems/form-largest-integer-with-digits-that-add-up-to-target/submissions/340200571
*/
    ...
    string helper(string cur, int target,vector<int>& cost){
        ...
        for(int i = 8; i>=0; i--){
            if(cost[i] <= target){
                ...
                string s2 = helper(cur + to_string(i+1), target-cost[i], cost);
                ans = MAX(ans, s2);
