...
Submission Url: https://leetcode.com/problems/max-area-of-island/submissions/747357328
*/
...
public:
    ...
    void dfs(vector<vector<int>>& grid, vector<vector<bool>>& vis, int i, int j, int &count){
        ...
        for(int a=0;a<4;a++){
            int x = i + X[a];
            int y = j + Y[a];
            if(isInsideGrid(x, y, grid.size(), grid[0].size()) and grid[x][y] == 1  and vis[x][y] == false){
