...
Submission Url: https://leetcode.com/problems/all-paths-from-source-to-target/submissions/867868033
*/

class Solution {
public:
    void findAllPaths(int source, int target, vector<int> &path, vector<vector<int>>& graph, vector<vector<int>>& ans){
        // base case: if source is equal to target, add the path to the answer and return
        if(source == target){
            ans.push_back(path);
            return;
        }
