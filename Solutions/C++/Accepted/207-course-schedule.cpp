...
Submission Url: https://leetcode.com/problems/course-schedule/submissions/346378866
*/

class Solution {
public:
    void dfs(vector<int>& vis, int node, vector<vector<int>>& tree, int ci, bool& ans){
        if(ans == false){
            return;
        }
        vis[node] = ci;
