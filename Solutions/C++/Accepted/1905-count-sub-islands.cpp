...
Submission Url: https://leetcode.com/problems/count-sub-islands/submissions/510417599
*/
...
public:
    ...
    int X[4] = {1,0,-1,0};
    int Y[4] = {0,1,0,-1};
    void dfs(int i, int j, int k, vector<vector<int>>& vis, vector<vector<int>>& grid){
       
        vis[i][j] = k;
        for(int x=0;x<4;x++){
            int ni = X[x] + i;
            int nj = Y[x] + j;
            if(inLim(ni,nj, grid.size(), grid[0].size()) and vis[ni][nj] == 0 and grid[ni][nj] == 1 ){
                dfs(ni,nj,k,vis,grid);
            }
        }
        
    }
    void dfs1(int i, int j, int k, vector<vector<int>>& vis, queue<pair<int,int>>& q, vector<vector<int>>& grid){
        int X[] = {1,0,-1,0};
        int Y[] = {0,1,0,-1};
        vis[i][j] = k;
        q.push({i,j});
